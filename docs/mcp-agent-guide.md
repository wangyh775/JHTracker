...
通过 MCP 协议，外部现代 AI 智能体（如 Claude Desktop、OpenCode、Cursor、Cline 等）能够以结构化、高安全、带审计的方式协同求职者开展岗位多维检索、人在环路推荐打分、外网职位采集与专岗 ATS 简历优化。
...
sequenceDiagram
    ...
    Agent->>MCP: 4. 调用 job_get_detail(job_id="job_001")
    MCP->>PubDB: 获取完整 JD、任职要求及官网申请链接
    MCP-->>Agent: 返回完整岗位元数据

    Agent->>MCP: 5. 调用 resume_optimize(job_id="job_001", save_as_version=True)
...
```mermaid
graph TD
    subgraph AllowedZone ["🟢 允许智能体调用的 8 大安全能力"]
        A1["岗位全景检索与详情下钻 (job_search / job_get_detail)"]
        A2["人在环路契合度计算 (job_recommend)"]
        A3["求职意向正负反馈记录 (job_feedback)"]
        A4["外网新岗位去重入库 (job_add_external)"]
        A5["定向爬虫增量同步触发 (job_sync_run)"]
        A6["只读简历技能画像查询 (resume_get_profile)"]
        A7["独立版本 ATS 专岗定制 (resume_optimize)"]
    end

    subgraph ForbiddenZone ["🔴 严苛禁止越权的红线操作"]
        F1["🚫 严禁推进/篡改投递看板阶段 (无 application_update_status 接口)"]
        F2["🚫 严禁覆写/覆盖用户原始简历 (原始版本只读保护)"]
        F3["🚫 严禁删除或清空看板投递记录 (用户主权独占)"]
        F4["🚫 严禁注入未经校验的恶意字符 (SAFE_IDENTIFIER_PATTERN 防护)"]
    end
```

- **决策主权在人**：投递看板状态（投递/笔试/面试/Offer）是高度严肃的人生决策，**智能体无权流转看板状态**。
...
## 3. 完整 FastMCP Tools 工具链规格参考

| 工具名称 | 输入参数 | 功能说明与约束 | 返回结果规范 |
| :--- | :--- | :--- | :--- |
| `job_search` | `query: str`<br>`city: Optional[str]`<br>`limit: int = 10` | 基于 FTS5 引擎全文模糊检索公共岗位库，自动过滤空值。 | 包含 `total` 命中数与 `jobs` 简要元数据列表。 |
| `job_get_detail` | `job_id: str` | 依据职位唯一 ID，提取完整职责、任职要求与官方网申直达链接。 | 完整职位字典对象（包含 `url`、`description` 等）。 |
| `job_recommend` | `resume_id: Optional[str]`<br>`limit: int = 5` | 基于用户简历技能余弦度与 HITL 特征权重计算契合度最高的岗位。 | 推荐列表（包含 `score` 匹配分值与 `match_reason` 解释）。 |
| `job_feedback` | `job_id: str`<br>`action: str`<br>`reason: Optional[str]` | 提交对岗位的感兴趣 (`ACCEPT`) 或跳过 (`REJECT`) 决策，驱动权重自学习。 | 操作状态与反馈持久化确认结果。 |
| `job_add_external`| `company: str`<br>`title: str`<br>`url: str`<br>`description: str` 等 | 将外部招聘渠道、社交媒体发现的新岗位去重录入公共库。 | 录入状态、生成哈希 ID 与是否为新增条目。 |
| `job_sync_run` | `spider_name: str` | 触发运行指定的定向招聘网站爬虫（如 `fangzhou`）。 | 爬虫采集统计数据与增量抓取报告。 |
| `resume_get_profile`| `resume_id: Optional[str]` | 只读提取指定简历或默认活跃简历的技能词云与摘要画像。 | 包含 `resume_id`、`title` 与 `skills` 标签列表。 |
| `resume_optimize` | `job_id: Optional[str]`<br>`resume_id: Optional[str]`<br>`save_as_version: bool = True` | 诊断简历与目标 JD 的差距，按 STAR 原则生成建议并可选独立存为新版本。 | 包含 `ats_score`、`suggestions`、优化内容与新生成的 `version_id`。 |
...
**用户提问**：
> “我对 ID 为 `job_fangzhou_550e8400` 的大厂核心平台岗非常感兴趣，请帮我做一份专岗定制简历！”

**智能体自动化调用链路**：
1. 智能体调用 `job_get_detail(job_id="job_fangzhou_550e8400")` 深入了解高频考点。
2. 智能体调用 `resume_optimize(job_id="job_fangzhou_550e8400", save_as_version=True)` 触发 ATS 评分与 STAR 强化。
3. 后端服务自动在 `~/.JHTracker/user_data.db` 中创建带有 `AI_OPTIMIZED` 标签的新简历。
4. 智能体向用户汇报：“已生成针对该岗位的独立定制版本（ID: `res_ai_xxx`），重点量化了分布式缓存与高并发 QPS 指标；原简历保持安全无损，可随时在 Web 端审阅预览！”
