...
print("Trace Start Result:", res_start)
...
for rev in reviews:
    ...
    print(f"[{cid}] Update Score -> {res_score}")
    
    # 2. Create application if new position provided
    app_res = None
    if rev["new_position"]:
        ...
        print(f"[{cid}] Create Application -> {app_res}")
...
print("Review Trace Result:", res_rev)
...
print("Complete Trace Result:", res_comp)

with open(r"D:/DJTU/HermesWorkspace/career-tracker/scripts/review_execution_summary_20260823.json", "w", encoding="utf-8") as f:
    json.dump({
        "task_id": task_id,
        "agent_name": agent_name,
        "start_trace": res_start,
        "results": results,
        "review_trace": res_rev,
        "complete_trace": res_comp
    }, f, ensure_ascii=False, indent=2)

print("\n--- ALL REVIEW ACTIONS EXECUTED AND RECORDED SUCCESSFULLY ---")
