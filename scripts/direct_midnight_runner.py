"""Direct Database Executor for Midnight Company Review"""
import os
import json
import sqlite3
...
try:
    ...
    if 'form_type' not in existing_cols:
        cur.execute("ALTER TABLE applications ADD COLUMN form_type VARCHAR(50)")
    if 'source_platform' not in existing_cols:
        cur.execute("ALTER TABLE applications ADD COLUMN source_platform VARCHAR(50)")
    ...
    if 'agent_reason' not in existing_cols:
        cur.execute("ALTER TABLE applications ADD COLUMN agent_reason TEXT")
    if 'agent_task_id' not in existing_cols:
        cur.execute("ALTER TABLE applications ADD COLUMN agent_task_id VARCHAR(100)")
    if 'source_url' not in existing_cols:
        cur.execute("ALTER TABLE applications ADD COLUMN source_url VARCHAR(500)")
    conn.commit()
except Exception as e:
    print("Schema update error (ignored):", e)

def record_trace(task_id, agent_name, event_type, payload, status="running"):
    ...
    if task_row:
        task_pk = task_row['id']
        cur.execute("UPDATE agent_tasks SET status = ?, agent_name = ? WHERE id = ?", (status, agent_name, task_pk))
    else:
        cur.execute(
    ...
    cur.execute(
        ...
        (task_pk, event_type, payload_str)
    )
    conn.commit()
...
for item in reviews:
    ...
    # 2. Check existing application for deduplication
    ...
    if existing:
        app_id = existing['id']
        app_created = False
        print(f"[{cname}] Application already exists (ID: {app_id}), skipped duplicate creation.")
    else:
        # Create new application proposal
        cur.execute("""
        ...
        app_created = True
        print(f"[{cname}] Created new application proposal (ID: {app_id}).")

    # Record company review trace
    record_trace(
        task_id=task_id,
        agent_name=agent_name,
        event_type="company_review_completed",
        payload={
            "company_id": cid,
            "company_name": cname,
            "new_score": new_score,
...
out_summary_file = os.path.join(BASE_DIR, 'scripts', 'midnight_review_20260822_result.json')
with open(out_summary_file, "w", encoding="utf-8") as f:
    json.dump(execution_summary, f, ensure_ascii=False, indent=2)

print("\nAll tasks completed successfully. Output written to:", out_summary_file)
conn.close()
