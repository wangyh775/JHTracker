...
# Candidate targets for rotation review
reviews = [
    {
        "company_id": 775,
        "name": "杭州宇树科技有限公司",
...
for item in reviews:
    ...
    score_res_raw = update_company_score(company_id=cid, score=new_score, reason=score_reason)
    score_res = json.loads(score_res_raw)
    print(f"[{cname}] Score update:", score_res)

    # 2. Create application proposal
    app_res_raw = create_application(
        company_id=cid,
        ...
        resume_id=pos_data["resume_id"],
        form_type=pos_data.get("form_type", "open_question"),
        source_platform=pos_data.get("source_platform", "official")
    )
    app_res = json.loads(app_res_raw)
    print(f"[{cname}] Application create:", app_res)
    ...
    execution_summary.append({
        ...
        "match_score": pos_data["match_score"],
        "source_url": pos_data["source_url"],
        "app_created": app_res.get("created", False),
        "app_id": app_res.get("application", {}).get("id")
    })

# 4. Final finish trace
res_finish = record_agent_trace(
    task_id=task_id,
    agent_name=agent_name,
    event_type="complete",
    payload={
        "summary": "成功完成 5 家轮询企业 (宇树科技、中望软件、国望光学、格见半导体、普渡科技) 的 2027 届校招深度复盘，更新评分并新增 5 条有效投递记录。",
...
print("\nMidnight Review Execution Finished Successfully!")
