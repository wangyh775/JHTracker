...
class FeishuJobsAutofillAdapter(BaseAutofillAdapter):
    """飞书招聘（jobs.feishu.cn）系统智能预填适配器"""

    async def can_handle(self, url: str, page_content: str) -> bool:
        return bool(re.search(r"jobs\.feishu\.cn", url, re.I))
    ...
    async def fill_form(self, page, profile: Dict[str, Any], resume_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        mapping_rules = [
            ...
            (r"期望城市|意向城市|工作地点|city", b.get("target_city", "深圳/杭州/上海/苏州")),
            (r"项目名称|课题名称", p_auto.get("name", "大型高温FDM 3D打印设备分布式控制系统研发")),
            (r"担任角色|职务", p_auto.get("role", "课题负责人")),
            (r"项目描述|经历描述", p_auto.get("description", "")),
            (r"自我评价|自我介绍|个人总结", "大连交大机械硕士(2027届)。具备500mm高温FDM整机结构设计、EPLAN电气规划及MPC/EKF控制算法落地经验；发表EI论文1篇，申请发明专利1项，获国家一等奖。")
        ]
        ...
        for el in inputs:
            try:
                ...
                label_text = ""
                parent = await el.evaluate_handle("node => node.closest('.form-item, .semi-form-field, div, section')")
        ...
        return {
            "success": True,
            "platform": "Feishu Jobs (飞书招聘)",
            "filled_fields": filled_count,
            "uploaded_resume": uploaded_resume,
            "message": "飞书招聘网申表单已安全预填完毕，请核实后手动提交。",
